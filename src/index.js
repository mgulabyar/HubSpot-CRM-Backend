...
app.post("/api/hubspot/contacts", async (req, res) => {
  try {
    ...
    const contactProperties = cleanProperties({
      firstname,
      lastname,
      email,
      phone,
...
app.get("/api/hubspot/contacts/:id/notes", async (req, res) => {
  try {
    ...
    const noteIds =
      associationResponse.data.results?.map((item) => item.id) || [];
    ...
    const notes = noteResponses
      .map((response) => response.data)
      .sort((first, second) => {
        const firstDate =
          first.properties?.hs_timestamp || first.updatedAt || "";
...
app.patch("/api/hubspot/contacts/:id", async (req, res) => {
  await updateObjectById("contacts", req, res);
});
...
app.post("/api/hubspot/deals", async (req, res) => {
  try {
    const properties = pickProperties(req.body, allowedDealProperties);
...
app.patch("/api/hubspot/deals/:id", async (req, res) => {
  try {
    ...
    if (Object.keys(properties).length === 0) {
      return res.status(400).json({
        success: false,
        message: "At least one valid deal property is required",
      });
    }
